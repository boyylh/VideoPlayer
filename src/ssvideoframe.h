...
class SSVideoFrame : public DragAbleWidget, public VideoPlayerCallBack
{
    Q_OBJECT
...
public:
    ...
    explicit SSVideoFrame(QWidget *parent = 0);
    ~SSVideoFrame();

    void setVideoId(enumVideoId videoId);
    void setVideoFile(const QString &sFileName);
    void synPlay();
    void synPause();
    void synSeek(int v);
    void synSetVolume(int v);
    bool IsOpenFile();
