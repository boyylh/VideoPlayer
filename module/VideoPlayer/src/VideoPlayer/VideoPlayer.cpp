...
void VideoPlayer::loadVideoFile(const QString &sFileName)
{
    if(!sFileName.isEmpty())
        mFilePath = sFileName.toStdString();
    else
        mFilePath = "";
}
...
void VideoPlayer::openAudio()
{
    //    if(!AudioPlayer::Get()->IsStartAudio())
    //    {
    //        AudioPlayer::Get()->sampleRate = in_sample_rate;
    //        AudioPlayer::Get()->channel = aCodecCtx->channels;
    //        AudioPlayer::Get()->sampleSize = 16;
    //        AudioPlayer::Get()->Start();//启动音频
    //        qDebug()<<"音频启动成功";
    //    }
}
